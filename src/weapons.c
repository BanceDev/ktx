...
void W_Precache(void) {
	...
	if (cvar("k_instagib_custom_models")) {
		trap_precache_sound(
			"weapons/coilgun.wav"); // player railgun for instagib
	}
}
...
/*
 ...
 Used by coilgun for Instagib mode, bullet doesn't stop at first player, it goes
 through till it hits wall
 ================
 */
...
void FireInstaBullet(vec3_t dir, deathType_t deathtype) {
	...
	for (ignore = self, depth = 0; depth < 32; depth++) {
		...
		VectorScale(dir, 4, tmp);
		VectorSubtract(g_globalvars.trace_endpos, tmp,
					   puff_org); // puff_org = trace_endpos - dir*4;
		...
		if (solid != SOLID_SLIDEBOX) {
			break; // obstacle not a player/monster, probably something solid
...
void FireBullets(float shotcount, vec3_t dir, float spread_x, float spread_y,
				 float spread_z, deathType_t deathtype) {
	vec3_t direction;
	vec3_t src, tmp, tmp2;
	qbool classic_shotgun = cvar("k_classic_shotgun");
	qbool non_random_bullets =
		(k_yawnmode || (!match_in_progress && self && (self->ct == ctPlayer) &&
						iKey(self, "nrb")));
	...
	while (shotcount > 0) {
		...
		//		direction = dir + crandom()*spread[0]*v_right +
		//crandom()*spread[1]*v_up;
		if (cvar("k_instagib")) {
...
void W_FireShotgun(void) {
	...
	float offset;
	int bullets = bound(1, cvar("k_hitboxcheck_bullets"), 64),
		color = iKey(self, "railcolor"), b;
	...
	// make puff
	traceline(PASSVEC3(src), src[0] + s_dir[0], src[1] + s_dir[1],
			  src[2] + s_dir[2], false, self);
	VectorScale(dir, 4, tmp);
	VectorSubtract(g_globalvars.trace_endpos, tmp, puff_org);

	// fire each bullet
	for (offset = -(float)(bullets - 1) / 2, b = bullets; b > 0;
		 b--, offset++) {
...
void FixQuad(gedict_t *owner) {
	...
	// at the same time that still allow apply full quad damage to nearby
	// players in case of "quad bore"
...
void T_InstaKickback(void) {
	...
	VectorAdd(self->s.v.origin, tmp, self->s.v.origin) ent_remove(self);
}
...
void T_MissileTouch(void) {
	...
	VectorAdd(self->s.v.origin, tmp, self->s.v.origin)
		// self->s.v.origin = self->s.v.origin - 8 *
		// normalize(self->s.v.velocity);
		WriteByte(MSG_MULTICAST, SVC_TEMPENTITY);
...
void GrenadeTouch(void) {
	...
	sound(self, CHAN_WEAPON, "weapons/bounce.wav", 1,
		  ATTN_NORM); // bounce sound
...
void superspike_touch(void) {
	...
	if (other->s.v.takedamage) {
		...
		T_Damage(other, self, PROG_TO_EDICT(self->s.v.owner),
				 (k_yawnmode ? 16 : 18));
	} else {
		WriteByte(MSG_MULTICAST, SVC_TEMPENTITY);
...
void W_SetCurrentAmmo(void) {
	...
	if ((self->think == (func_t)player_stand1) ||
	...
	if (need_fix) {
		...
		// does't change ->s.v.frame and ->walkframe, so we does't break current
		// animation sequence on weapon change
...
float W_BestWeapon(void) {
	...
	if (!strnull(w_rank)) {
		while (*w_rank) {
			...
			if ((weapon == 8) && (it & IT_LIGHTNING) &&
				(self->s.v.ammo_cells >= 1)) {
				return IT_LIGHTNING;
			}

			if ((weapon == 7) && (it & IT_ROCKET_LAUNCHER) &&
				(self->s.v.ammo_rockets >= 1)) {
				return IT_ROCKET_LAUNCHER;
			}

			if ((weapon == 6) && (it & IT_GRENADE_LAUNCHER) &&
				(self->s.v.ammo_rockets >= 1)) {
				return IT_GRENADE_LAUNCHER;
			}

			if ((weapon == 5) && (it & IT_SUPER_NAILGUN) &&
				(self->s.v.ammo_nails >= 2)) {
				return IT_SUPER_NAILGUN;
			}

			if ((weapon == 4) && (it & IT_NAILGUN) &&
				(self->s.v.ammo_nails >= 1)) {
				return IT_NAILGUN;
			}

			if ((weapon == 3) && (it & IT_SUPER_SHOTGUN) &&
				(self->s.v.ammo_shells >= 2)) {
				return IT_SUPER_SHOTGUN;
			}

			if ((weapon == 2) && (it & IT_SHOTGUN) &&
...
void W_Attack(void) {
	...
	case IT_AXE:
		if (self->ctf_flag & CTF_RUNE_HST) {
			self->attack_finished =
				g_globalvars.time + 0.5 - (cvar("k_ctf_rune_power_hst") / 10);
			HasteSound(self);
		} else {
	...
	case IT_SUPER_SHOTGUN:
		...
		if (self->ctf_flag & CTF_RUNE_HST) {
			self->attack_finished =
				g_globalvars.time + 0.5 - (cvar("k_ctf_rune_power_hst") / 20);
			HasteSound(self);
		} else {
			self->attack_finished =
				g_globalvars.time + (k_yawnmode ? 0.8 : 0.7);
		}
...
qbool W_ChangeWeapon(int wp) {
	...
	switch (wp) {
	case 1:
		// ctf shortcut for newbs: selecting axe when you already have it
		// switches to grapple
...
qbool CycleWeaponCommand(void) {
	...
	for (i = 0; i < 20;
		 i++) // qqshka, 20 is just from head, but prevent infinite loop
	{
		am = 0;
		switch ((int)self->s.v.weapon) {
		case IT_LIGHTNING:
...
int CaptainImpulses(void) {
	...
	if ((self->s.v.impulse > MAX_CLIENTS) || !capt_num(self)) {
		return 0; // s: return 0 if captain mode is set and no captain things
				  // were entered
	}
