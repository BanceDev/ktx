...
float k_attendees;	 // stores number of players on server - used in 'ready'
					 // checking
...
float k_checkx =
	0; // global which set to true when some time spend after match start
float k_force;	  // used in forcing matchstart
float k_maxspeed; // used to store server maxspeed to allow switching by admins
float k_oldmaxspeed; // used to store old value of maxspeed prior to freezing
					 // the map
...
float k_nochange = 0; // used to indicate if frags changes somehow since last
					  // time 'scores' command was called
...
float match_over; // boolean - whether or not the match stats have been printed
				  // at end of game
...
// { CTF
int k_ctf_custom_models; // if server has flag/grapple models you can enable
						 // them here
...
int k_allowed_free_modes; // reflect appropriate cvar - but changed only at map
						  // load
...
float k_sv_locktime; // some time before non VIP players can't connect,
					 // spectators not affected
