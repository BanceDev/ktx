...
qbool CheckRate(gedict_t *p, char *newrate) {
	...
	// This is used to check a players rate. If above allowed setting then it
	// kicks em off.
	player_rate =
		atof(strnull(newrate) ? (newrate = ezinfokey(p, "rate")) : newrate);

	if (strnull(newrate)) {
		return false; // empty rate is special, with new mvdsv mean maximum
					  // allowed, so allow it
	}
...
void CheckTiming(void) {
	...
	timing_players_time =
		timing_players_time ? timing_players_time : 6; // 6 is default
	...
	for (p = world; (p = find_plr(p));) {
		if ((p->k_lastPostThink + timing_players_time) < g_globalvars.time) {
			...
			// ok we are detect - player lagged, so do something, effects is
			// exception
			if (firstTime) {
				if (timing_players_action & TA_INVINCIBLE) {
					...
					p->s.v.solid = 0;
					p->s.v.movetype = 0;
					SetVector(p->s.v.velocity, 0, 0,
							  0); // speed is zeroed and not restored
				}
			}
		...
		if (p->k_timingWarnTime) {
			if (timing_players_action & TA_GLOW) {
				// we can't set this in CheckLightEffects() because
				// CheckLightEffects() will not called if client lagged
				p->s.v.effects = (int)p->s.v.effects | EF_DIMLIGHT;
			}
		} else {
			// moved to CheckLightEffects(), I means CheckLightEffects() will
			// turn EF_DIMLIGHT off
		}
	}
}
...
void PrewarParams(void) {
	...
	g_globalvars.parm1 = IT_AXE | IT_SHOTGUN | IT_SUPER_SHOTGUN | IT_NAILGUN |
...
void NonDMParams(void) {
	if (ISDEAD(self)) {
		// NOTE: player will have "empty" backpack if map changin and he/she was
		// dead...
		...
		self->s.v.items =
			...
			~(IT_KEY1 | IT_KEY2 | IT_INVISIBILITY | IT_INVULNERABILITY |
			  IT_SUIT | IT_QUAD | IT_SUPERHEALTH);
...
void SetChangeParms(void) {
	...
	// we must use k_matchless cvar here because it can be changed during game
	// somehow (via direct server conlose etc) If matchless mode is not active,
	// set just ordinary prewar stats
...
void execute_changelevel(void) {
	...
	intermission_exittime =
		g_globalvars.time + 1 + max(1, cvar("demo_scoreslength"));
...
void changelevel_touch(void) {
	...
	if (deathmatch) {
		if (isCTF()) {
			...
			// TODO: qqshka: I have an idea to teleport players to
...
void SP_trigger_changelevel(void) {
	...
	// qqshka: yeah, treat k_remove_end_hurt as hint to remove some shit from
	// this level,
...
void ClientKill(void) {
	...
	if (isCA() && match_in_progress) {
		if ((ra_match_fight != 2) || ca_round_pause) {
			...
			self->can_respawn =
				false; // No respawning after suicide in wipeout mode
			self->seconds_to_respawn = 999; // no countdown
		}
	}

	if (isCTF() && (match_in_progress == 2) &&
		((g_globalvars.time - match_start_time) < 10)) {
		G_sprint(self, PRINT_HIGH,
				 "Can't suicide during first 10 seconds of CTF match\n");
...
float CheckSpawnPoint(vec3_t v) { return false; }
...
// K_SPW_0_NONRANDOM changes "Normal QW respawns" to "pre-qtest nonrandom
// respawns"
#ifdef K_SPW_0_NONRANDOM
	if (k_spw == 0) {
		static gedict_t *last_spot =
			g_edicts; // basically, g_edicts is same as world, but we can't
					  // initialize static variable with world.
	...
	for (spot = world; (spot = find(spot, FOFCLSN, spawnname));) {
		...
		for (thing = world;
			 (thing = trap_findradius(thing, spot->s.v.origin, 84));) {
			if ((thing->ct != ctPlayer) || ISDEAD(thing) || (thing == self)) {
				continue; // ignore non player, or dead played, or self
			}

			// k_spw 2 and 3 and 4 feature, if player is spawned not far away
			// and run around spot - treat this spot as not valid. k_1spawn
			// store this "not far away" time. k_1spawn is _also_ set after
			// player passed teleport
			if (!(((k_spw == 2) || (k_spw == 3) || (k_spw == 4)) &&
				  (match_in_progress == 2) &&
		...
		if (!k_yawnmode && k_spw && (k_spw != 4) && (match_in_progress == 2) &&
			...
			pcount++; // ignore this spot in this case, protection from spawn
	...
	if (match_in_progress ==
		2) // protect(in some case) player from be spawnfragged for some time
	{
		...
		if (!match_in_progress || k_spw == 1 || (k_spw == 2 && !k_checkx)) {
			...
			trap_makevectors(
				isRA()
					? spot->mangle
					: spot->s.v
						  .angles); // stupid ra uses mangles instead of angles

			for (thing = world;
				 (thing = trap_findradius(thing, spot->s.v.origin, 84));) {
				if ((thing->ct != ctPlayer) || ISDEAD(thing) ||
					(thing == self)) {
					continue; // ignore non player, or dead played, or self
				}
				...
				if (g_globalvars.trace_fraction < 1) {
					VectorMA(g_globalvars.trace_endpos, -35.0,
							 g_globalvars.v_forward, v1);
				}
	...
	// Because of array limits, fall back to old spawn model on maps with more
	// than MAX_SPAWN_WEIGHTS spots
	// - Molgrum
	if (k_yawnmode && (match_in_progress == 2) &&
		(totalspots <= MAX_SPAWN_WEIGHTS)) {
		float f;
		gedict_t *spawnp;
		int i = 0;
		...
		for (i = 0; i < numspots; i++) {
			if (spot == spawnp) {
				...
				self->spawn_weights[(int)spot->cnt] +=
					(f / (float)(numspots - 1));
			}
...
gedict_t *SelectSpawnPoint(char *spawnname) {
	gedict_t *k_lastspawn = self->k_lastspawn; // we need remember this before
											   // calling Sub_SelectSpawnPoint()
	gedict_t *spot = Sub_SelectSpawnPoint(spawnname);

	// k_spw 4 feature, recheck spawn poit second time if we select same spawn
	// point in row, so it low chance to get same spawn point
...
qbool CanConnect(void) {
	...
	if (k_sv_locktime && !VIP(self)) {
		...
		// no checks in matchLess mode, in bloodfest, or if there is no game in
		// progress

		if (!k_bloodfest || !match_in_progress) {
			// in non bloodfest mode always announce but do not announce during
			// bloodfest round.
		...
		// don't allow anyway
		G_sprint(self, 2,
				 "Match in progress, server locked\n"
				 "Please reconnect as spectator\n");

		// can't connect
		return false;
	} else if ((cvar("k_lockmode") == 1) || isCA()) {
		// allow players in existing teams, but different behavior for
		// team/duel/ffa/CA
		if (isDuel() || isFFA()) {
			...
			for (from = 0, p = world; (p = find_plrghst(p, &from));) {
				if (p != self && streq(getteam(p), t)) {
					break; // don't kick, find "player" or "ghost" with equal
						   // team
				}
			}
	...
	if (isCTF() &&
		(strneq(getteam(self), "red") && strneq(getteam(self), "blue"))) {
		G_sprint(self, 2,
				 "Match in progress,\n"
	...
	for (usrid = 1; usrid < k_userid;
		 usrid++) // search for ghost for this player (localinfo)
	{
	...
	if (usrid < k_userid) // ghost probably found (localinfo)
	{
		...
		if (p) // found ghost entity
		{
			...
			// In CA, if current team doesn't match old team then just don't
			// restore stats/gamestate Otherwise restore frags and set ca_ready
			else if (isCa && !teamEqual) {
				// if player's team isn't what it was before, then he will be a
				// "dead" player until the match is over
				self->ca_ready = 0;
				...
				self->ca_ready =
					isCa ? p->ca_ready
						 : 0; // return to the game if playing clan arena

				if (isCa && !self->ca_ready) {
					G_bprint(2, "%s entered the game\n", self->netname);
				} else if (isTeam() || isCTF()) {
					self->k_teamnum =
						p->k_teamnum; // we alredy have team in localinfo
					G_bprint(2, "%s \220%s\221 %s %d %s%s\n", self->netname,
							 getteam(self), redtext("rejoins the game with"),
							 (int)self->s.v.frags, redtext("frag"),
							 redtext(count_s(self->s.v.frags)));
				} else {
					self->k_teamnum = 0; // force check is we have team in
										 // localinfo or not below
					G_bprint(2, "%s %s %d %s%s\n", self->netname,
							 redtext("rejoins the game with"),
							 (int)self->s.v.frags, redtext("frag"),
							 redtext(count_s(self->s.v.frags)));
				}
...
void ClientConnect(void) {
	...
	if (isCTF()) {
		int red =
			0; // Keeps track of which team and colors to set for new player

		// If matchless CTF, make sure team is red or blue
		if (k_matchLess) {
			if (!(streq(getteam(self), "red") ||
				  streq(getteam(self), "blue"))) {
				// Compare number of blue and red players. If red > blue (i.e.
				// red > 0), set new player to blue.
				gedict_t *p;
				for (p = world; (p = find_plr(p));) {
			...
			G_sprint(self, PRINT_HIGH,
					 "To change teams, /disconnect, /team red or /team blue, "
					 "then /reconnect\n");
		}
	...
	// if bloodfest is active then set player as unready and kill him later in
	// PutClientInServer() if match in progress then set client ready anyway. if
	// there matchless mode then set client ready too.
	if (k_bloodfest) {
		self->ready = 0;
	} else {
	...
	if (k_captains == 2) { // in case of team picking, check if there is a free
						   // spot for player number 1-16
		int i;

		for (i = 1, p = world; p && i <= MAX_CLIENTS; i++) {
			for (p = world;
				 (p = find_plr(p)) && (p == self || p->s.v.frags != i);) {
				; // empty
			}
		}
	...
	if (isRA()) {
		ra_in_que(self); // put cleint in ra queue, so later we can move it to
						 // loser or winner
	}

	// Yawnmode: reset spawn weights at server join (can handle max
	// MAX_SPAWN_WEIGHTS spawn points atm) Just count the spots
	...
	// Don't use this spawn model for maps with more than MAX_SPAWN_WEIGHTS
	// spawns (shouldn't even happen)
...
void PutClientInServer(void) {
	...
	self->on_hook = false;
	self->hook_out = false;
	self->maxspeed = cvar("sv_maxspeed"); // qqshka - ctf stuff, discard haste
										  // rune modifier after u die
	self->regen_time = -1;
	...
	if (isCA() && cvar("k_clan_arena") == 3) {
		self->th_die = FT_PlayerFreeze;
	} else {
	...
	if (isHoonyModeAny() && (spot = HM_choose_spawn_point(self))) {
		// Nothing more to do
			 ...
			 match_in_progress && streq(mapname, "povdmm4")) {
		gedict_t *highest = NULL;
		for (spot = world; (spot = ez_find(spot, "info_player_deathmatch"));) {
			if ((highest == NULL) ||
				(spot->s.v.origin[2] > highest->s.v.origin[2])) {
				highest = spot;
			}
		}
		...
		// first spawn in CTF on corresponding base, later used
		// info_player_deathmatch. qqshka: I found that it sux and added
		// variable which force players spawn ONLY on the base, so maps like
		// qwq3wcp9 works fine!

		if (isCTF() && ((match_start_time == g_globalvars.time) ||
						(cvar("k_ctf_based_spawn") == 1))) {
			spot = SelectSpawnPoint(streq(getteam(self), "red")
										? "info_player_team1"
										: "info_player_team2");
		}
		// Pick between neutral spawn points in the mid of the map and spawn
		// points within home base to avoid the fish in a barrel problem where
		// flag position is overrun by str/quad/pent and players are instagibbed
		// over and over again.
		else if (isCTF() && (cvar("k_ctf_based_spawn") == 2)) {
			spot = SelectSpawnPoint(g_random() <= 0.5 ? "info_player_deathmatch"
									: streq(getteam(self), "red")
										? "info_player_team1_deathmatch"
										: "info_player_team2_deathmatch");
		...
		spot =
			SelectSpawnPoint(coop ? "info_player_coop" : "info_player_start");
	}
	...
	if (spot->s.v.items && isHoonyModeAny()) {
		...
		if (!match_in_progress) {
			int itemvalues[] = {IT_SUPER_SHOTGUN,	IT_NAILGUN,
								IT_SUPER_NAILGUN,	IT_GRENADE_LAUNCHER,
								IT_ROCKET_LAUNCHER, IT_LIGHTNING};
			...
			G_sprint(self, 2, "  %d%s, %dh\n", (int)armorvalue,
					 armorExplanation, (int)spot->s.v.health);
		...
		if (match_in_progress == 2) {
			...
			// If any megahealth items are set to this spawn, set to track this
			// player
			if (!strnull(spot->targetname)) {
				gedict_t *p;

				for (p = world; (p = find(p, FOFCLSN, "item_health"));) {
					if (p->s.v.spawnflags == 2 &&
						streq(spot->targetname, p->target)) {
						// Pretend item was taken by this player
						p->model = "";
						p->s.v.solid = SOLID_NOT;
						p->s.v.nextthink =
							g_globalvars.time +
							max(0, min(p->initial_spawn_delay, 5)) +
							0.2; // bit extra otherwise 1 frame after the other
	...
	if ((deathmatch == 4 || k_bloodfest) && (match_in_progress == 2)) {
		...
		// 0 evalutes to DMM4_INVINCIBLE_DEFAULT, negative value disable
		// invincible
		dmm4_invinc_time =
			(dmm4_invinc_time ? bound(0, dmm4_invinc_time, DMM4_INVINCIBLE_MAX)
							  : DMM4_INVINCIBLE_DEFAULT);
	...
	if (isCTF()) {
		...
		if (cvar("k_ctf_ga") && deathmatch < 4 && match_in_progress == 2) {
			...
			self->s.v.items =
				(int)self->s.v.items | IT_ARMOR1; // add green armor
		}
	}
...
int tiecount(void) { return (deathmatch == 4 ? 2 : 3); }
...
void PlayerJump(void) {
	...
	if ((self->spawn_time + 0.05) > g_globalvars.time) {
		self->s.v.velocity[2] =
			-270; // discard +jump till 50 ms after respawn, like ktpro
	...
	if (self->s.v.waterlevel >= 2) {
		...
		if (self->swim_flag < g_globalvars.time) {
			...
			// Yawnmode: don't play swimming sound to cut older clients without
			// smartjump some slack
			// - Molgrum
			if (!k_yawnmode) {
				if (g_random() < 0.5) {
	...
	if (!self->brokenankle) {
		...
		// JUMPBUG[
		// the engine checks velocity_z and won't perform the jump if it's <
		// zero!

		// qqshka - i know this is shit - but ktpro does it, and probably server
		// engine even ignor this hack
...
void MakeGhost(void) {
	...
	ghost->ghost_clr |=
		(int)bound(0, iKey(self, "bottomcolor"), 13); // save colors
...
void ClientDisconnect(void) {
	...
	if (!(CountPlayers() - CountBots())) {
		...
		// Well, not quite sure if it OK, k_matchLess C global variable really
		// must be set ONCE per map. At the same time, k_matchless cvar should
		// be set ONCE per whole server run, so it should be OK. So here we try
		// to put server back to non matchless mode.
		...
		if (old_matchless != k_matchLess) {
			...
			changelevel(mapname); // force reload current map in practice mode
								  // anyway, ASAP
		}
#ifdef BOT_SUPPORT
		else if (bots_enabled() != old_bots_enabled) {
			changelevel(mapname); // need to reload map, dimensions of entities
								  // will need to change
		}
#endif
		else {
			Spawn_DefMapChecker(10); // delayed map reload, may be skipped due
...
void Print_Wp_Stats(void) {
	...
	gedict_t *g =
		self->ct == ctSpec ? PROG_TO_EDICT(self->s.v.goalentity) : NULL;
	gedict_t *e = self->ct == ctPlayer
					  ? self
					  : (g ? g : world); // stats of whom we want to show
...
#endif
	float sg = wps & S_SG ? 100.0 * e->ps.wpn[wpSG].hits /
								max(1, e->ps.wpn[wpSG].attacks)
						  : 0;
	float ssg = wps & S_SSG ? 100.0 * e->ps.wpn[wpSSG].hits /
								  max(1, e->ps.wpn[wpSSG].attacks)
							: 0;
	float ng = wps & S_NG ? 100.0 * e->ps.wpn[wpNG].hits /
								max(1, e->ps.wpn[wpNG].attacks)
						  : 0;
	float sng = wps & S_SNG ? 100.0 * e->ps.wpn[wpSNG].hits /
								  max(1, e->ps.wpn[wpSNG].attacks)
							: 0;
#if 0 /* percentage */
...
#endif
	float lg = wps & S_LG ? max(0.001, 100.0 * e->ps.wpn[wpLG].hits /
										   max(1, e->ps.wpn[wpLG].attacks))
						  : 0;
	...
	if ((e == world) || (e->ct != ctPlayer)) {
		...
		G_centerprint(self, "%s%s", buf,
					  redtext("Tracking no one (+wp_stats)"));
	...
	if (ktpl) {
		if (lg) {
			strlcat(buf, lg ? va("%s:%.1f ", redtext("lg"), lg) : "",
					sizeof(buf));
		}

		if (rl) {
			strlcat(buf, rl ? va("%s:%.0f ", redtext("rl"), rl) : "",
					sizeof(buf));
		}

		if (gl) {
			strlcat(buf, gl ? va("%s:%.0f ", redtext("gl"), gl) : "",
					sizeof(buf));
		}

		if (axe) {
			strlcat(buf, axe ? va("%s:%.0f ", redtext("axe"), axe) : "",
					sizeof(buf));
		}

		if (sg) {
			strlcat(buf, sg ? va("%s:%.1f ", redtext("sg"), sg) : "",
					sizeof(buf));
		}

		if (ssg) {
			strlcat(buf, ssg ? va("%s:%.1f ", redtext("ssg"), ssg) : "",
					sizeof(buf));
		}

		if (ng) {
			strlcat(buf, ng ? va("%s:%.1f ", redtext("ng"), ng) : "",
					sizeof(buf));
		}

		if (sng) {
			strlcat(buf, sng ? va("%s:%.1f ", redtext("sng"), sng) : "",
					sizeof(buf));
		}
	} else {
		if (lg) {
			wp_wrap_cat(lg ? va("%s:%.1f", redtext("lg"), lg) : "", buf,
						sizeof(buf));
		}

		if (rl) {
			wp_wrap_cat(rl ? va("%s:%.0f", redtext("rl"), rl) : "", buf,
						sizeof(buf));
		}

		if (gl) {
			wp_wrap_cat(gl ? va("%s:%.0f", redtext("gl"), gl) : "", buf,
						sizeof(buf));
		}

		if (axe) {
			wp_wrap_cat(axe ? va("%s:%.0f", redtext("axe"), axe) : "", buf,
						sizeof(buf));
		}

		if (sg) {
			wp_wrap_cat(sg ? va("%s:%.1f", redtext("sg"), sg) : "", buf,
						sizeof(buf));
		}

		if (ssg) {
			wp_wrap_cat(ssg ? va("%s:%.1f", redtext("ssg"), ssg) : "", buf,
						sizeof(buf));
		}

		if (ng) {
			wp_wrap_cat(ng ? va("%s:%.1f", redtext("ng"), ng) : "", buf,
						sizeof(buf));
		}

		if (sng) {
			wp_wrap_cat(sng ? va("%s:%.1f", redtext("sng"), sng) : "", buf,
						sizeof(buf));
		}
	}
...
/*
 * Function is called when a player or spectator enables continuous score
 * display with +scores console command.
 * */
void Print_Scores(void) {
	char buf[1024] = {0}, *last_va;

	int i, minutes = 0, seconds = 0, ts = 0, es = 0,
		   ls = iKey(self, "ls") + (iKey(self, "ktpl") ? 12 : 0);
	...
	if (isCTF()) {
		...
		if (flag1 && flag2) {
			...
			strlcat(buf, va("%s \205%s\205%s\205 ", rune, r_f, b_f),
					sizeof(buf));
		}
	}
	...
	if (k_sudden_death) {
		strlcat(buf,
				va("%s:%-5.5s", redtext("tl"),
				   redtext(k_sudden_death == SD_NORMAL ? "sd" : "tb")),
				sizeof(buf));
	} else {
		strlcat(buf, va("%s:%02d:%02d", redtext("tl"), minutes, seconds),
				sizeof(buf));
	}
	...
	if (sc_ok) {
		if (isCTF()) {
			...
			if ((current_umode < um2on2on2) || (current_umode > um4on4on4)) {
				strlcat(buf,
						last_va = va("  \364:%d  \345:%d  \x90%d\x91", ts, es,
									 ts - es),
						sizeof(buf));
			} else {
				// if the current UserMode is 2on2on2, 3on3on3, 4on4on4, we have
				// 3 teams
				int s1 = get_scores1();
				int s2 = get_scores2();
				int s3 = get_scores3();
				...
				if (streq(t1, t)) {
					...
					strlcat(buf,
							last_va = va("  \364:%d  \345:%d  \x90%d\x91  "
										 "\345:%d  \x90%d\x91",
	...
	// add spaces, so line in most cases is don't move from side to side during
	// frags changes
	if ((i = (isCTF() ? sizeof("  [-zzzzz]") - 1
					  : sizeof("  t:xxxxx  e:yyyyy  [-zzzzz]") - 1) -
			 strlen(last_va)) > 0) {
		int offset = strlen(buf);
...
void ZeroFpsStats(void) {
	// zero these so the average/highest FPS is calculated for each delay
	// period.
...
void PlayerPreThink(void) {
	...
	if (self->sc_stats && self->sc_stats_time &&
		(self->sc_stats_time <= g_globalvars.time) &&
		(match_in_progress != 1) && !isRACE()) {
		Print_Scores();
	}

	if (self->wp_stats && self->wp_stats_time &&
		(self->wp_stats_time <= g_globalvars.time) &&
		(match_in_progress != 1) && !isRACE()) {
		Print_Wp_Stats();
	}
	...
	if (self->fDisplayIllegalFPS < g_globalvars.time && framechecks &&
		...
		// client uptime check
		// code by Zibbo
		r = self->fAverageFrameTime * 100 /
			(g_globalvars.time - self->real_time);

		if ((r > 103) && !match_in_progress) {
			G_sprint(self, PRINT_HIGH,
					 "Warning: It seems that your machine has a too long "
					 "uptime causing a bug in your QW client. Please restart "
					 "your machine and fix this message.\n");
		...
		if (self->uptimebugpolicy > 3) {
			G_bprint(PRINT_HIGH, "\n%s gets kicked for too long uptime\n",
					 self->netname);
			G_sprint(self, PRINT_HIGH,
					 "Please reboot your machine to get rid of the problem\n");
			stuffcmd(self, "disconnect\n"); // FIXME: stupid way
		}
		// ends here
		...
		{
			float peak =
				self->fLowestFrameTime ? (1.0f / self->fLowestFrameTime) : 1;
			...
			if (self->fIllegalFPSWarnings > 3) {
				...
				G_bprint(PRINT_HIGH, "%s gets kicked for potential cheat\n",
						 self->netname);
				G_sprint(self, PRINT_HIGH,
						 "Please reboot your machine to try to get rid of the "
						 "problem\n");
				stuffcmd(self, "disconnect\n"); // FIXME: stupid way
			}
		}
	...
	if (self->s.v.deadflag == DEAD_DYING) {
		...
		// invoked on death for some reason (couldn't figure out why). This
		// leads to a state when the player stands still after dying and can't
		// respawn or even suicide and has to reconnect. This is checked and
		// fixed here
		if ((g_globalvars.time > (self->dead_time + 0.1)) &&
			((self->s.v.frame < 41) ||
			 (self->s.v.frame > 102))) // FIXME: hardcoded range of dead frames
		{
			StartDie();
		}
	...
	if ((g_globalvars.time > self->attack_finished) &&
		(self->s.v.currentammo == 0) && (self->s.v.weapon != IT_AXE) &&
	...
	if (self->ctf_flag & CTF_RUNE_RGN) {
		if (self->regen_time < g_globalvars.time) {
			...
			if (self->s.v.health < 150) {
				...
				self->regen_time +=
					1 / ((cvar("k_ctf_rune_power_rgn") / 2) + 1);
#ifdef BOT_SUPPORT
...
void CheckPowerups(void) {
	...
	if (self->invisible_finished) {
		...
		if (self->invisible_finished < g_globalvars.time) { // just stopped
			...
			adjust_pickup_time(&self->it_pickup_time[itRING],
							   &self->ps.itm[itRING].time);
	...
	if (self->invincible_finished) {
		...
		if (self->invincible_finished < g_globalvars.time) { // just stopped
			self->s.v.items -= IT_INVULNERABILITY;
			self->invincible_time = 0;
			self->invincible_finished = 0;

			adjust_pickup_time(&self->it_pickup_time[itPENT],
							   &self->ps.itm[itPENT].time);
	...
	if (self->super_damage_finished) {
		...
		if ((self->super_damage_finished < g_globalvars.time) &&
			...
			adjust_pickup_time(&self->it_pickup_time[itQUAD],
							   &self->ps.itm[itQUAD].time);
	...
	if (self->radsuit_finished) {
		...
		if (self->radsuit_finished < g_globalvars.time) { // just stopped
...
void CheckLightEffects(void) {
	...
	if (!match_in_progress && !match_over && !k_matchLess && !self->ready &&
...
void WS_CheckUpdate(gedict_t *p) {
	...
	for (i = wpNONE + 1; i < wpMAX; i++) {
		...
		for (j = 0; j < trackers_cnt; j++) {
			stuffcmd_flags(trackers[j], STUFFCMD_IGNOREINDEMO,
						   "//wps %d %s %d %d\n", NUM_FOR_EDICT(p) - 1,
...
void PlayerPostThink(void) {
	...
	if (intermission_running) {
		setorigin(self, PASSVEC3(intermission_spot->s.v.origin));
		SetVector(self->s.v.velocity, 0, 0,
				  0); // don't stray off the intermission spot too far
	...
	CheckPowerups();
	CheckLightEffects(); // NOTE: guess, this must be after CheckPowerups(), so
						 // u r warned.
	CheckStuffRune();
...

	{
		...
		if (!match_in_progress && !match_over && !k_captains && !k_matchLess &&
			...
			if (iKey(self, "kf") & KF_SPEED) {
				...
				self->s.v.armorvalue =
					(int)(velocity < 1000 ? velocity + 1000 : -velocity);
				self->s.v.frags = (int)(velocity) / 1000;
				self->s.v.ammo_shells =
					100 + (int)(velocity_vert_abs) / 100000000;
				self->s.v.ammo_nails =
					100 + (int)(velocity_vert_abs) % 1000000 / 10000;
				self->s.v.ammo_rockets =
					100 + (int)(velocity_vert_abs) % 10000 / 100;
...
void SendTeamInfo(gedict_t *t) {
	...
	for (cnt = 0, p = world; (p = find_plr(p));) {
		...
		if (t->trackent && t->trackent == NUM_FOR_EDICT(p)) {
			continue; // we pseudo speccing such player, no point to send info
					  // about him
		}
		...
		if (strnull(
				nick = ezinfokey(p, "k_nick"))) // get nick, if any, do not send
												// name, client can guess it too
		{
			nick = ezinfokey(p, "k");
		}
		...
		stuffcmd_flags(t, STUFFCMD_IGNOREINDEMO,
					   "//tinfo %d %d %d %d %d %d %d \"%s\" %d %d %d %d\n", cl,
					   (int)p->s.v.origin[0], (int)p->s.v.origin[1],
					   (int)p->s.v.origin[2], h, a, (int)p->s.v.items, nick,
...
void CheckTeamStatus(void) {
	...
	if (isRACE()) {
		return; // could advance in the future by working out ongoing positions
				// and sending those?
	}

	if ((g_globalvars.time - lastTeamLocationTime) <
	...
	for (p = world; (p = find_client(p));) {
		...
		// check for ezquake or user specifie use team info even non ezquake
		// client
...
void SendSpecInfo(gedict_t *spec, gedict_t *target_client) {
	...
	if (spec) // if spec has a value, we only want to send that spec's info
	{
		...
		tr = NUM_FOR_EDICT(PROG_TO_EDICT(spec->s.v.goalentity)) -
		...
		for (t = world; (t = find_spc(t));) {
			...
			tr = NUM_FOR_EDICT(PROG_TO_EDICT(t->s.v.goalentity)) -
				 1; // num for player spec is tracking

			if (target_client && target_client != t) {
				stuffcmd_flags(target_client, STUFFCMD_IGNOREINDEMO,
							   "//spi %d %d\n", cl, tr);
...
void StatsHandler(gedict_t *targ, gedict_t *attacker) {
	...
	if (isDuel()) {
		...
		if (targ->control_start_time) {
			targ->ps.control_time +=
				g_globalvars.time - targ->control_start_time;
	...
	if (attacker->ct == ctPlayer) {
		...
		if (targ == attacker) {
		...
		} else if ((isTeam() || isCTF()) && streq(targteam, attackerteam) &&
			...
			// hmm, may be add some priority? so if targ have rl and gl bump
			// only wpn[wpRL].ekills ?
			if ((items & IT_AXE)) {
...
// Instagib rewards are suspended till I figure out if that would be useful or
// not -- deurk static int	i_agmr_height = 0; // used for instagib, reset to 0
// on each map reload...
...
float Instagib_Obituary(gedict_t *targ, gedict_t *attacker) {
	...
	traceline(PASSVEC3(targ->s.v.origin), targ->s.v.origin[0],
			  targ->s.v.origin[1], targ->s.v.origin[2] - 2048, true, targ);
	...
	if ((int)attacker->s.v.flags & FL_ONGROUND) {
		if ((playerheight >= 250) && (playerheight < 400)) {
			G_bprint(2, "%s from %s: height %d\n", redtext("AirGib"),
					 attacker->netname, (int)playerheight);
		} else if ((playerheight >= 400) && (playerheight < 1000)) {
			G_bprint(2, "%s from %s: height %d\n", redtext("Great AirGib"),
					 attacker->netname, (int)playerheight);
		} else if (playerheight >= 1000) {
			G_bprint(2, "%s from %s: height %d\n", redtext("Amazing AirGib"),
					 attacker->netname, (int)playerheight);
		}

		if (playerheight > 45) {
			attacker->ps.i_height += playerheight;
			attacker->ps.i_maxheight =
				max(attacker->ps.i_maxheight, playerheight);
	...
	/* Instagib rewards are suspended till I figure out if that would be useful
	 or not -- deurk if ( attacker->ps.i_height > 2000 )
	 {
	 if ( !i_agmr_height )
	 {
	 if ( !attacker->i_agmr )
...
void ClientObituary(gedict_t *targ, gedict_t *attacker) {
	...
	if (k_bloodfest && !targ->ready) {
		return; // someone connecting during round of bloodfest and got pseudo
				// death.
	}
	...
	if (attacker->ct ==
		ctPlayer) // so, inside this "if" targ and attacker is players
	{
		if (targ == attacker) {
			// killed self
			...
			if (dtGL == targ->deathtype) {
				...
				deathstring = va(" electrocutes %s\n", g_himself(targ));
					   ...
					   targ->deathtype) { // similar code present in case where
			...
			if ((dtTELE1 != targ->deathtype) || cvar("k_tp_tele_death")) {
				// -1 frag always if non "teledeath", and -1 on "teledeath" if
				// allowed also relax this rules on first seconds of match
				if ((g_globalvars.time - match_start_time) > 1) {
					attacker->s.v.frags -= 1;
					logfrag(attacker, attacker); // ZOID 12-13-96: killing a
												 // teammate logs as suicide
				}
			}
			...
			return;
		} else { // normal kill, Kteams version
			...
			if (dtTELE1 == targ->deathtype) {
				...
				deathstring =
					(targ->s.v.health < -40 ? " was gibbed by " : " rides ");
				deathstring2 = "'s rocket\n";

				if ((attacker->super_damage_finished > 0) &&
					(targ->s.v.health < -40) && (!cvar("k_midair"))) {
					switch ((int)(g_random() * 3)) {
					case 0:
			...
			if (!cvar("k_midair")) {
				G_bprint(PRINT_MEDIUM, "%s%s%s%s", victimname, deathstring,
						 attackername, deathstring2);
			}
		}
...
qbool PlayerCanPause(gedict_t *p) {
	...
	// Check for matchtag. If it is set, it is an official (probably), so pause
	// might be allowed.
