...
void player_chain4(void) {
	// Original ctf grapple used frame 73 here, but that causes problems with
	// cl_deadbodyfilter 2 Frame 139 is a decent alternative especially given
	// that 73 never looked good anyway self->s.v.frame = 73;
	self->s.v.frame = 139;
...
// this shit present in ktpro, but test does't show why we need this,
// because 99.9% of time diff is just 0
...
void PainSound(void) {
	...
	if (((self->s.v.watertype == CONTENT_WATER) ||
		 (self->s.v.watertype == CONTENT_SLIME)) &&
...
void player_pain(struct gedict_s *attacker, float take) {
	...
	if (isCA() && (streq(getteam(self), getteam(attacker)) || self->no_pain)) {
		return; // No pain when shooting teammates in CA or when respawning
				// (wipeout)
	}
...
void VelocityForDamage(float dm, vec3_t v) {
	...
	if (vlen(damage_inflictor->s.v.velocity) > 0) {
		...
		//  v = v + (25 *
		//  normalize((self->s.v.origin)-damage_inflictor->s.v.origin));
...
gedict_t *ThrowGib(char *gibname, float dm) {
	...
	newent->s.v.nextthink =
		g_globalvars.time + (k_short_gib ? 2 : (10 + g_random() * 10));
	newent->s.v.frame = 0;
	newent->s.v.flags = 0;
...
void GibPlayer(void) {
	qbool bloodfest_round_connect =
		(k_bloodfest &&
		 !self->ready); // in case of bloodfest and connecion during round.
	gedict_t *p;
	...
	if (TELEDEATH(self)) {
		...
		sound(p, CHAN_VOICE,
			  (g_random() < 0.5 ? "player/gib.wav" : "player/udeath.wav"), 1,
			  ATTN_NONE);
	}
}
...
void PlayerDie(void) {
	...
	if ((self->s.v.health < -40) || (dtSQUISH == self->deathtype) ||
		(dtSUICIDE == self->deathtype) || isRA()) {
		GibPlayer();

		// Yawnmode: respawn has the same delay (900ms) regardless of deathtype
		// gib/normal
		// - Molgrum

		// Hoonymode: Also force some time, e.g. to prevent instant respawn
		// after /kill which can cause bug if kill telefrags an idle player
		// (counts as two points...) only ever happens in testing, but oh well
		// --phil
...
void StartDie(void) {
	...
	if (1) {
		// Yawnmode: exclude diea1 and diec1 so the respawn time is always 900
		// ms
		switch (i_rnd(1, 3)) {
		case 1:
			player_dieb1();
		...
		// Note that this generates random values in 1..6 range, so player_diee1
		// is executed twice as often as other death sequences. Dunno if this
		// should be fixed -- Tonik
