...
static float loser_respawn_time =
	999; // number of seconds before a teammate would've respawned
...
int CA_get_score_1(void) { return team1_score; }

int CA_get_score_2(void) { return team2_score; }
...
int calc_respawn_time(gedict_t *p, int offset) {
	...
	multiple = bound(3, teamsize + 1, 6); // first respawn won't take more than
										  // 6 seconds regardless of team size
	...
	if (isWipeout && (p->round_deaths + offset <= max_deaths)) {
		...
		time = p->round_deaths + offset == 1
				   ? multiple
				   : (p->round_deaths - 1 + offset) * (multiple * 2);
	}
...
float last_alive_time(gedict_t *player) {
	...
	// because we only want to play the audio once at the begining of the
	// countdown Only play the audio if the player is alive
	if (!player->last_alive_active && (time > 0) && player->in_play) {
		...
		if (player->last_alive_active && player->in_play) {
			player->escape_time =
				g_globalvars.time; // start timer for escape time
		}
...
float team_last_alive_time(int team) {
	...
	float time = 999;
	char *team_name =
		team ? (team == 1 ? cvar_string("_k_team1") : cvar_string("_k_team2"))
			 : "";
...
qbool isCA(void) { return (isTeam() && cvar("k_clan_arena")); }
...
void track_player(gedict_t *observer) {
	...
	if (player && !observer->in_play && observer->tracking_enabled) {
		if (observer->track_target && observer->track_target->in_play) {
			...
			else if (streq(getteam(observer),
						   getteam(observer->track_target))) {
				player = observer->track_target;
			}
		...
		upward_distance = 0;
		observer->hideentity =
			EDICT_TO_PROG(player); // in this mode we want to hide player model
								   // for watcher's view
		VectorCopy(player->s.v.v_angle, observer->s.v.angles);
		// }

		observer->s.v.fixangle =
			true; // force client v_angle (disable in 3rd person view)

		trap_makevectors(player->s.v.angles);
		VectorMA(player->s.v.origin, follow_distance, g_globalvars.v_forward,
				 observer->s.v.origin);
		VectorMA(observer->s.v.origin, upward_distance, g_globalvars.v_up,
				 observer->s.v.origin);

		// avoid positionning in walls
		traceline(PASSVEC3(player->s.v.origin), PASSVEC3(observer->s.v.origin),
				  false, player);
		VectorCopy(g_globalvars.trace_endpos, observer->s.v.origin);

		if (g_globalvars.trace_fraction == 1) {
			VectorCopy(g_globalvars.trace_endpos, observer->s.v.origin);
			VectorMA(observer->s.v.origin, 10, g_globalvars.v_forward,
					 observer->s.v.origin);
		} else {
			VectorSubtract(g_globalvars.trace_endpos, player->s.v.origin,
						   delta);
...
void CA_PutClientInServer(void) {
	...
	if (match_in_progress == 2) {
		...
		if (!self->teamcolor && self->ca_ready) {
			...
			else if (streq(cvar_string("_k_team1"), "red") ||
					 streq(cvar_string("_k_team2"), "red")) {
				self->teamcolor = "13";
			}
			// if a "blue" team exists, you aren't on it, so set color to red
			else if (streq(cvar_string("_k_team1"), "blue") ||
					 streq(cvar_string("_k_team2"), "blue")) {
				self->teamcolor = "4";
			}
			// neither "red" nor "blue" teams exist, so default to team1 being
			// red and team2 blue
			else {
				self->teamcolor =
					streq(cvar_string("_k_team1"), getteam(self)) ? "4" : "13";
			}
		}

		SetUserInfo(self, "topcolor", self->teamcolor ? self->teamcolor : "0",
					0);
		SetUserInfo(self, "bottomcolor",
					self->teamcolor ? self->teamcolor : "0", 0);
	...
	if (ISDEAD(self) && cvar("k_clan_arena") != 3) {
		...
		self->in_limbo = (self->ca_ready) &&
						 (self->round_deaths <= max_deaths) &&
						 self->can_respawn;
		self->spawn_delay = 0;
...
qbool CA_can_fire(gedict_t *p) {
	...
	return (ISLIVE(p) && (ra_match_fight == 2) && time_to_start &&
			(g_globalvars.time >= time_to_start));
}
	...
	if (self->ct == ctPlayer && !match_in_progress) {
		if (!self->ready) {
			G_centerprint(self, "Welcome to %s\n\n\n%s %s", mode,
						  "set your team and type", redtext("/ready"));
		} else {
...
// sends player state information to client for use in teaminfo or other hud
// elements
void CA_SendTeamInfo(gedict_t *t) {
	int cl;
	int cnt;
	...
	for (cnt = 0, p = world; (p = find_plr(p));) {
		...
		if (p->ca_ready ||
			match_in_progress != 2) // be sure to send info if in prewar
		{
			if (match_in_progress == 2) {
				if (ISLIVE(p)) {
		...
		if (streq(tm, getteam(p))) {
			...
			origin0 = origin1 = origin2 = h = a = items = shells = nails =
				rockets = cells = 0;
		}
		...
		stuffcmd(t,
				 "//cainfo %d %d %d %d %d %d %d \"%s\" %d %d %d %d %d %d %d %d "
				 "%d %d\n",
				 cl, origin0, origin1, origin2, h, a, items, nick, shells,
				 nails, rockets, cells, camode, deadtype, timetospawn, kills,
				 deaths, track_target);
	}
}

// wipeout: check if dying player survived just long enough for teammate to
// spawn
...
// return 1 if there one alive team and alive_team point to 1 or 2 wich refering
// to _k_team1 or _k_team2 cvars return 2 if there at least two alive teams
static int CA_check_alive_teams(int *alive_team) {
	...
	for (p = world; (p = find_plr(p));) {
		if (!first_team) {
			if (ISLIVE(p)) {
				first_team =
					getteam(p); // ok, we found first team with alive players
			}
		...
		if (strneq(first_team, getteam(p))) {
			if (ISLIVE(p)) {
				few_alive_teams =
					true; // we found at least two teams with alive players
				break;
			}
		}
	}
...
void team_round_summary(int alive_team) {
	...
	G_bprint(
		...
		"%-6s %4d %4d %s\n"
		"\n",
		redtext("------ ---- ---- ------------"), team1, t1_score, t1_need,
...
void print_player_stats(qbool series_over) {
	...
	for (p = world; (p = find_plr(p));) {
		if (p->ready && (streq(getteam(p), cvar_string(va("_k_team1"))) ||
...
void CA_OnePlayerStats(gedict_t *p, qbool series_over) {
	...
	if (!use_totals) {
		round_elg = 100 * (h_lg - p->ca_round_lghit) /
					max(1, a_lg - p->ca_round_lgfired);
	}

	snprintf(score, sizeof(score), "%.0f",
			 use_totals ? p->s.v.frags : p->s.v.frags - p->ca_round_frags);
	snprintf(damage, sizeof(damage), "%.0f",
			 use_totals ? dmg_g : dmg_g - p->ca_round_dmg);
	snprintf(dmg_took, sizeof(dmg_took), "%.0f",
			 use_totals ? dmg_t : dmg_t - p->ca_round_dmgt);
	snprintf(kills, sizeof(kills), "%.0f",
			 use_totals ? rkills : rkills - p->ca_round_kills);
	snprintf(deaths, sizeof(deaths), "%.0f",
			 use_totals ? p->deaths : p->deaths - p->ca_round_deaths);
	snprintf(gl_hits, sizeof(gl_hits), "%.0f",
			 use_totals ? vh_gl : vh_gl - p->ca_round_glhit);
	snprintf(rl_hits, sizeof(rl_hits), "%.0f",
			 use_totals ? vh_rl : vh_rl - p->ca_round_rlhit);
	snprintf(rl_directs, sizeof(rl_directs), "%.0f",
			 use_totals ? h_rl : h_rl - p->ca_round_rldirect);
	snprintf(lg_eff, sizeof(lg_eff), "%.0f", use_totals ? e_lg : round_elg);

	G_bprint(
		2, "%3s %5s %4s %2s %2s %3s %3s %3s %3s%s %s\n",
		strneq(score, "0") ? score : "-", strneq(damage, "0") ? damage : "-",
		strneq(dmg_took, "0") ? dmg_took : "-",
		strneq(kills, "0") ? kills : "-", strneq(deaths, "0") ? deaths : "-",
		strneq(gl_hits, "0") ? gl_hits : "-",
		strneq(rl_hits, "0") ? rl_hits : "-",
		strneq(rl_directs, "0") ? rl_directs : "-",
		strneq(lg_eff, "0") ? lg_eff : "-",
		strneq(lg_eff, "0") ? redtext("%") : " ", getname(p));
...
void EndRound(int alive_team) {
	...
	if (!ca_round_pause) {
		...
		loser_respawn_time =
			loser_team ? team_last_alive_time(loser_team) : 999;
	}
	...
	if (pause_count <= 0) {
		...
		if (pause_count <= 7) {
			if (!alive_team) {
				...
				snprintf(round_or_series, sizeof(round_or_series), "%s",
						 ((alive_team == 1 &&
						   team1_score == (CA_wins_required() - 1)) ||
						  (alive_team == 2 &&
						   team2_score == (CA_wins_required() - 1)))
							 ? "series"
							 : "round");
				...
				if ((loser_respawn_time < 2) && (loser_respawn_time > 0)) {
					G_cp2all("Team \x90%s\x91 wins the %s!\n\n\nTeam %s needed "
							 ...
							 round_or_series,
							 cvar_string(va("_k_team%d", loser_team)),
							 loser_respawn_time);
				} else {
					G_cp2all("Team \x90%s\x91 wins the %s!",
							 cvar_string(va("_k_team%d", alive_team)),
					...
					if (cvar("k_clan_arena") == 2) {
						...
						if (alive_team &&
							streq(getteam(p),
								  cvar_string(va("_k_team%d", alive_team))) &&
							p->in_play) {
							if (streq(ezinfokey(p, "topcolor"), "13") &&
								streq(ezinfokey(p, "bottomcolor"), "13")) {
								p->super_time = 8;
				...
				for (p = world; (p = find_plr(p));) {
					if (p->in_play &&
						...
						G_bprint(2, "%s %s %.0f%s%.0f\n", p->netname,
								 redtext("had"), p->s.v.armorvalue,
								 redtext("/"), p->s.v.health);
					}
				}
...
void CA_spectator_think(void) {
	...
	if (p->ct == ctPlayer && !p->in_play && p->tracking_enabled) {
		// if the player you're observing is following someone else, hide the
		// player model
	...
	if (p->ct == ctPlayer) {
		if (match_in_progress == 2 && ra_match_fight == 2 && round_time > 2 &&
			!ca_round_pause) {
...
void CA_Frame(void) {
		...
		for (p = world; (p = find_plr(p));) {
			...
			if (p->in_limbo) {
				...
				if (p->seconds_to_respawn <= 0) {
					...
					p->time_of_respawn =
						g_globalvars.time; // resets alive_time to 0
				} else {
					if (!p->tracking_enabled) {
						snprintf(
							p->cptext, sizeof(p->cptext),
							"\n\n\n\n\n\n\n\n\n%d\n\n\n seconds to respawn\n",
							(int)ceil(p->seconds_to_respawn));
						G_centerprint(p, "%s", p->cptext);
					}
			...
			// both you and the enemy are the last alive on your team
					 ...
					 e_last_alive) {
				snprintf(str_last_alive, sizeof(str_last_alive), "%d",
						 last_alive);
				snprintf(p->cptext, sizeof(p->cptext),
						 "\n\n\n\n\n\n%s\n\n\n%s\n\n\n\n", redtext("1 on 1"),
						 last_alive == 999 ? " " : redtext(str_last_alive));

				G_centerprint(p, "%s", p->cptext);
			}
			// you're the last alive on your team versus two or more enemies...
			// hide!
			else if (p->in_play && p->alive_time > 2 && last_alive) {
				snprintf(str_last_alive, sizeof(str_last_alive), "%d",
						 last_alive);
			...
			// only one enemy remains... find him!
				...
				snprintf(str_e_last_alive, sizeof(str_e_last_alive), "%d",
						 e_last_alive);
				snprintf(p->cptext, sizeof(p->cptext),
						 "\n\n\n\n\n\n%s\n\n\n%s\n\n\n\n", "one enemy left",
						 e_last_alive == 999 ? " " : str_e_last_alive);
	...
	// check if there exist only one team with alive players and others are
	// eluminated, if so then its time to start ca countdown
	...
	if ((team1_score >= CA_wins_required()) ||
		(team2_score >= CA_wins_required())) {
		int winning_team;
		...
		if (!ra_match_fight) {
			...
			for (p = world; (p = find_plr(p));) {
				...
				G_sprint(p, 2, "round \x90%d\x91 starts in 5 seconds.\n",
						 round_num);
				snprintf(p->cptext, sizeof(p->cptext),
						 " "); // reset any server print from last round
			}
		}
		...
		if (r <= 0) {
			...
			if (r < 6) {
				...
				G_cp2all("%d\n\n\n \x90%s\x91:%s \x90%s\x91:%s\n", r,
						 cvar_string("_k_team1"), dig3(team1_score),
