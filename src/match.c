...
// It is weird because used string comparision so I treat it as slow and idiotic
// but it return more players than CountPlayers().
...
// else return 1 (even we have more members than memcnt, dunno is this bug <-
// FIXME)
...
void ListDemoMarkers(void) {
	...
	for (i = 0; i < demo_marker_index; ++i) {
		...
		G_bprint(2, "%s: %d:%02d \220%s\221\n", redtext("Time"), (total / 60),
				 (total % 60), demo_markers[i].markername);
	}

	G_bprint(2, "\235\236\236\236\236\236\236\236\236\236\236\236\236\236\236"
				"\236\236\236\236"
...
void EndMatch(float skip_log) {
	...
	if (/* skip_log || */ !deathmatch) {
		...
		if (is_real_match_end) {
			...
			lastscore_add(); // save game result somewhere, so we can show it
							 // later
		}
	}
...
void CheckOvertime(void) {
	// SM_PrepareShowscores() is called in StartMatch(), but in matchless mode
	// this happens when the first player joins. Therefore, teams != 2 yet and
	// the function doesn't do anything. This causes matchless CTF to go into
	// perpetual overtime, since "sc = get_scores1() - get_scores2();"
	// calculated below will always return 0. So, we call this function here to
	...
	k_exttime =
		bound(1, cvar("k_exttime"), 999); // at least some reasonable values

	// In hoonymode the round is timing out, not the match - we're effectively
	// always in suddendeath mode
	if (isHoonyModeAny()) {
		HM_draw();
	...
	// Overtime.
	// Ok we have now decided that the game is ending, so decide overtime wise
	// here what to do.
	...
	if ((isTeam() || isCTF()) && (teams != 2)) {
		k_mb_overtime =
			0; // no overtime in case of less than 2 or more than 2 teams
	} else if (((isDuel() || isFFA()) && ed1 && ed2) // duel or ffa
			   || ((isTeam() || isCTF()) && (teams == 2) &&
				   (players > 1))) // Handle a 2v2 or above team game or 1v1 CTF
	{
		if (((k_mb_overtime == 3) &&
			 abs(sc) > 1) // tie-break overtime allowed with one frag difference
						  // (c) ktpro
			|| ((k_mb_overtime != 3) &&
				abs(sc) > 0)) // time based or sudden death overtime allowed
							  // with zero frag difference
		{
			k_mb_overtime = 0;
		}
	} else {
...
void TimerThink(void) {
	...
	// if in matchless mode, check that the user hasn't exceeded
	// k_matchless_max_idle_time
	if (k_matchLess && CountPlayers() && match_in_progress &&
		k_matchLess_idle_time) {
		for (p = world; (p = find_plr(p));) {
			idle_time = (int)(g_globalvars.time - p->attack_finished);
			if (idle_time > k_matchLess_idle_time) {
				G_sprint(p, 2,
						 "You were forced to reconnect as spectator by "
						 "exceeding the maximum idle time of %i seconds.\n",
						 k_matchLess_idle_time);
				stuffcmd_flags(p, STUFFCMD_IGNOREINDEMO, "spectator 1\n");
				if (!strnull(ezinfokey(p, "Qizmo"))) {
					stuffcmd_flags(
						p, STUFFCMD_IGNOREINDEMO,
				...
				G_sprint(p, 2,
						 "\207%s You will be forced to spectate if you do not "
						 "fire within %i seconds!\n",
						 redtext("WARNING:"),
						 (k_matchLess_idle_time - k_matchLess_idle_warn));
			}
		}
	}
	...
	if ((self->k_teamnum < g_globalvars.time) && !k_checkx) {
		k_checkx = 1; // global which set to true when some time spend after
					  // match start
	}
		...
		if ((self->cnt2 == f1) && (self->cnt == f2)) {
			...
			for (p = world; (p = find_plr(p));) {
				if (strcmp(p->netname, "") && p->s.v.health > 0) {
					p->s.v.items =
						(int)p->s.v.items | (IT_QUAD | IT_INVULNERABILITY);
					p->super_time = 1;
		...
		if (k_showscores && !isCA()) {
			if ((current_umode < um2on2on2) || (current_umode > um4on4on4)) {
				...
				if ((s1 > s2) && (s1 > s3)) {
					...
					G_bprint(2, "%s \220%s\221 leads by %s frag%s\n",
							 redtext("Team"), cvar_string("_k_team1"),
							 dig3(abs((int)sc)), count_s(abs((int)sc)));
				} else if ((s2 > s1) && (s2 > s3)) {
					// Team 2 is leading
					if (s1 > s3) {
					...
					G_bprint(2, "%s \220%s\221 leads by %s frag%s\n",
							 redtext("Team"), cvar_string("_k_team2"),
							 dig3(abs((int)sc)), count_s(abs((int)sc)));
				} else if ((s3 > s1) && (s3 > s2)) {
					// Team 3 is leading
					if (s1 > s2) {
					...
					G_bprint(2, "%s \220%s\221 leads by %s frag%s\n",
							 redtext("Team"), cvar_string("_k_team3"),
							 dig3(abs((int)sc)), count_s(abs((int)sc)));
				} else {
					G_bprint(2, "The game is currently a tie\n");
				}
	...
	if ((self->cnt == 1) &&
		((self->cnt2 == 30) || (self->cnt2 == 15) || (self->cnt2 <= 10))) {
	...
	for (p = world; (p = nextent(p));) {
		...
		if (deathmatch >= 4) {
			if (streq(p->classname, "weapon_nailgun") ||
				...
				streq(p->classname, "weapon_grenadelauncher") ||
				streq(p->classname,
					  "weapon_lightning")) { // no weapons for any of this
											 // deathmatches (4 or 5)
				soft_ent_remove(p);
				continue;
			}
				...
				if (streq(p->classname, "item_shells") ||
					...
					(streq(p->classname, "item_health") &&
					 ((int)p->s.v.spawnflags &
					  H_MEGA))) { // no weapon ammo and megahealth for dmm4
					soft_ent_remove(p);
					continue;
				}
			}
		...
		if (k_killquad &&
			streq(p->classname,
				  "item_artifact_super_damage")) { // no normal quad in killquad
												   // mode.
			soft_ent_remove(p);
			continue;
		}
...
// See bug #115... if 2 players spawn next to each other, the first player hears
// the second, but the second only hears the first based on final location
// during pre-war
static void SM_ExecuteQueuedSpawnEffects(void) {
	gedict_t *p;
...
void SM_PrepareShowscores(void) {
	...
	if (k_matchLess &&
		!isCTF()) // skip this in matchLess mode, unless CTF matchless
				  // (otherwise causes unlimited overtime because "sc" is always
				  // = 0 in CheckOvertime())
	{
		return;
	}
	...
	if (((!isTeam() && !isCTF()) ||
		...
		!(isCTF() &&
		  k_matchLess)) // we need 2 ready teams, unless CTF matchless
	{
		return;
	}
...
void SM_PrepareHostname(void) {
	...
	cvar_set("_k_host",
			 cvar_string("hostname")); // save host name at match start

	if (k_showscores && !strnull(team1) && !strnull(team2)) {
		if ((current_umode < um2on2on2) || (current_umode > um4on4on4)) {
			cvar_set("hostname", va("%s (%.4s vs. %.4s)\207",
									cvar_string("hostname"), team1, team2));
		} else {
			char *team3 = cvar_string("_k_team3");

			cvar_set("hostname",
					 va("%s (%.4s vs. %.4s vs. %.4s)\207",
						cvar_string("hostname"), team1, team2, team3));
		}
	} else {
...
void StartMatch(void) {
	...
	SM_PrepareMap(); // remove/add some items from map regardind with dmm and
					 // game mode
	...
	self->k_teamnum =
		g_globalvars.time + 3; // dirty i know, but why waste space?
	...
	if (!self->cnt) {
		ent_remove(self); // timelimit == 0, so match will end no due to
						  // timelimit but due to fraglimit or something
	}
...
void PersonalisedCountdown(char *baseText) {
	...
	for (p = world; (p = find_plr(p)); /**/) {
		...
		if (p->k_hoony_new_spawn &&
			!strnull(p->k_hoony_new_spawn->targetname)) {
			strlcat(
				text,
			...
			if (timelimit) {
				...
				if (minutes == 0) {
					strlcat(text, va("%s %3ss\n", "Duration", dig3(seconds)),
							sizeof(text));
				} else if (seconds == 0) {
					strlcat(text, va("%s %3sm\n", "Duration", dig3(minutes)),
							sizeof(text));
				} else {
					strlcat(text,
							va("%s %1s:%2s\n", "Duration", dig3(minutes),
			...
			if (!strnull(world->hoony_defaultwinner)) {
				if (streq(p->k_hoony_new_spawn->targetname,
						  world->hoony_defaultwinner)) {
...
void PrintCountdown(int seconds) {
	...
	//
	// Deathmatch	x
	// Mode			D u e l | T e a m | F F A | C O O P | BLOODFST | C T F | RA
	// | CA Spawnmodel	KTX | bla bla bla // optional Antilag		On|Off
	// NoItems		On // optional
	// Midair		On // optional
	...
	strlcat(text, va("%s: %2s\n\n\n", redtext("Countdown"), dig3(seconds)),
			sizeof(text));
	...
	if (isCA()) {
		strlcat(text, va("%s %3s\n", "RoundWins", dig3(CA_wins_required())),
				sizeof(text));
	...
	if (!(isHoonyModeTDM() && HM_current_point())) {
		...
		if (!isRACE()) {
			strlcat(text,
					va("%s %4s\n", "Respawns",
					   respawn_model_name_short(cvar("k_spw"))),
					sizeof(text));
		}

		if (cvar("sv_antilag")) {
			strlcat(text,
					va("%s %5s\n", "Antilag", dig3((int)cvar("sv_antilag"))),
					sizeof(text));
		}
		...
		if (cvar("k_teamoverlay") && tp_num() && !isDuel() && !isRACE()) {
			strlcat(text, va("%s %3s\n", "TmOverlay", redtext("on")),
					sizeof(text));
		}

		if (!isRA() && (isTeam() || isCTF())) {
			strlcat(text, va("%s %4s\n", "Teamplay", dig3(teamplay)),
					sizeof(text));
		}
	}
	...
	if (isHoonyModeAny()) {
		...
		if (HM_rounds()) {
			strlcat(text,
					va(" %s     %2s/%2s\n", "Round",
					   dig3(HM_current_point() + 1), dig3(HM_rounds())),
					sizeof(text));
		}
	} else if (timelimit) {
		strlcat(text, va("%s %3s\n", "Timelimit", dig3(timelimit)),
	...
	if (!isRA() && Get_Powerups() && strneq("off", Get_PowerupsStr())) {
		strlcat(text, va("%s %4s\n", "Powerups", redtext(Get_PowerupsStr())),
				sizeof(text));
	}
	...
	if ((deathmatch == 4) && !cvar("k_midair") && !cvar("k_instagib") &&
		!strnull(nowp =
					 str_noweapon((int)cvar("k_disallow_weapons") & DA_WPNS))) {
		strlcat(text,
				va("\n%s %4s\n", "Noweapon",
				   redtext(nowp[0] == 32 ? (nowp + 1) : nowp)),
				sizeof(text));
	}
...
qbool isCanStart(gedict_t *s, qbool forceMembersWarn) {
	...
	if (isCTF() &&
		!k_matchLess) // In matchless CTF, the "this map does not support CTF
					  // mode" would get spammed constantly in an unsupported
					  // map. So, don't bother returning false.
	{
...
void ShowMatchSettings(void) {
	...
	//	G_bprint(2, "Spawnmodel: %s\n", redtext( respawn_model_name( cvar(
	//"k_spw" ) ) ));
...
char *CompilateDemoName(void) {
	...
	if (isRA()) {
		...
		} else if (cvar("k_clan_arena") == 2) {
			strlcat(demoname, "wipeout", sizeof(demoname));
		} else {
			strlcat(demoname, "freezetag", sizeof(demoname));
		}
		...
		if (isTeam()) {
			...
			// The parameter is (current_umode-1), because the UserModes_t enum
			// has `umUnknown` as first element, but the um_list[] array doesn't
			...
			if (strCurrentUmode != NULL) {
				...
				// Something is wrong with current_umode. Let's use the legacy
				// format
				strlcat(demoname, (clt ? va("%don%d", clt, clt) : "team"),
						sizeof(demoname));
			}
		} else {
		...
		strlcat(demoname, va("unknown_%d", (int)CountPlayers()),
				sizeof(demoname));
	}
...
void StartDemoRecord(void) {
	...
	// extralog should be set by easyrecord and if we skip recording we will
	// have it set to WRONG value. So this set it at least to something
	// reasonable ffs.
	...
	if (cvar("demo_tmp_record")) { // FIXME: TODO: make this more like ktpro
		...
		if (isRACE()) {
			...
			record =
				false; // don't try to record (segfault) when already recording
		} else {
			record = true;
		}
		...
		if (record) {
			if (!strnull(cvar_string("serverdemo"))) {
				localcmd("sv_democancel\n"); // demo is recording, cancel before
											 // new one
			}
...
void StartTimer(void) {
	...
	timer->cnt2 = max(
		3, (int)cvar("k_count")); // at the least we want a 3 second countdown

	if (isHoonyModeDuel() && (HM_current_point() > 0)) {
		timer->cnt2 =
			3; // first point gets usual 10 seconds, next points gets less
	}
...
void StopTimer(int removeDemo) {
	...
	if (k_standby) {
		// Stops the bug where players are set to ghosts 0.2 second to go and
		// countdown aborts. standby flag needs clearing (sturm)
		k_standby = 0;
	...
	// do not set to Standby during points, (unless its the final point of
	// course)
...
void PlayerReady(qbool startIdlebot) {
	...
	if (self->ct == ctSpec && !isRACE()) {
		...
		for (p = world;
			 (p = (match_in_progress ? find_spc(p) : find_client(p)));) {
			G_sprint(p, 2, "%s %s to play\n", self->netname, redtext("desire"));
		}

		CheckAutoXonX(g_globalvars.time < 10
						  ? true
						  : false); // force switch mode asap if possible after
									// some time spent
	...
	if (!isCanStart(NULL, false)) {
		return; // rules does't allow us to start match, idlebot ignored because
				// of same reason
	}
	...
	// we ignore "all players ready" and "at least two players ready" checks in
	// bloodfest mode.
	if (!k_bloodfest) {
...
void PlayerSlowReady(void) { PlayerReady(false); }

void PlayerFastReady(void) { PlayerReady(true); }
...
void PlayerBreak(void) {
	...
	if (!k_matchLess || k_bloodfest) {
		// try stop countdown. (countdown between hoony-mode points can't be
		// stopped, treat as standard break request).
		qbool can_stop_hoonymode =
			(!isHoonyModeAny() || HM_current_point() == 0);
		...
		if (match_in_progress == 1 && can_stop_hoonymode) {
			...
			if (p && p->cnt2 > 1) {
				...
				if (!k_matchLess || (k_bloodfest && (CountRPlayers() < 1))) {
					G_bprint(2, "%s %s\n", self->netname,
							 redtext("stops the countdown"));
					StopTimer(1);
				} else {
					G_bprint(2, "%s %s\n", self->netname,
							 redtext("is not ready"));
				}
			}
	...
	if (self->v.brk) {
		...
		G_bprint(2, "%s %s %s vote%s\n", self->netname, redtext("withdraws"),
				 redtext(g_his(self)),
	...
	G_bprint(
		2, "%s %s%s\n", self->netname,
		redtext(k_matchLess ? "votes for next map"
							: "votes for stopping the match"),
		((votes = get_votes_req(OV_BREAK, true)) ? va(" (%d)", votes) : ""));

	// show warning to player - that he can't stop countdown alone in matchless
	// mode.
