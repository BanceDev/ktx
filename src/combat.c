...
void BotPlayerKilledEvent(gedict_t *targ, gedict_t *attacker,
						  gedict_t *inflictor);
...
const int deathtype_strings_cnt =
	sizeof(deathtype_strings) / sizeof(deathtype_strings[0]);
...
qbool ISDEAD(gedict_t *e) { return !ISLIVE(e); }
...
qbool CanDamage(gedict_t *targ, gedict_t *inflictor) {
	...
	if (targ->s.v.movetype == MOVETYPE_PUSH) {
		traceline(PASSVEC3(inflictor->s.v.origin),
				  ...
				  0.5 * (targ->s.v.absmin[2] + targ->s.v.absmax[2]), true,
				  self);
	...
	traceline(PASSVEC3(inflictor->s.v.origin), PASSVEC3(targ->s.v.origin), true,
			  self);
	if (g_globalvars.trace_fraction == 1) {
		return true;
	}
	...
	traceline(PASSVEC3(inflictor->s.v.origin), targ->s.v.origin[0] + dif[0],
			  targ->s.v.origin[1] + dif[1], targ->s.v.origin[2] + dif[2], true,
			  self);
	if (g_globalvars.trace_fraction == 1) {
		return true;
	}
...
void Killed(gedict_t *targ, gedict_t *attacker, gedict_t *inflictor) {
	...
	if (self->ct == ctPlayer) {
		; // empty
	} else if ((self->s.v.movetype == MOVETYPE_PUSH) ||
			   (self->s.v.movetype == MOVETYPE_NONE)) { // doors, triggers, etc
		if (self->th_die) {
			self->th_die();
		}
	...
	if (((int)(self->s.v.flags)) & FL_MONSTER) {
		...
		self->monster_desired_spawn_time =
			(resp_time
				 ? g_globalvars.time + resp_time + resp_time * g_random() * 0.5
				 : 0);
	...
	if (!isHoonyModeAny()) {
		...
		if (fraglimit &&
			(((targ->s.v.frags >= fraglimit) && (targ->ct == ctPlayer)) ||
			 ((attacker->s.v.frags >= fraglimit) &&
			  (attacker->ct == ctPlayer)))) {
			EndMatch(0);
		}

		if (fraglimit && isTeam()) {
			// in team modes fraglimit can be a sum of frags of all players in
			// the team
...
static float newceil(float f) { return ceil(((int)(f * 1000.0)) / 1000.0); }
...
void MidairDamageBonus(gedict_t *attacker, gedict_t *targ, float midheight) {
	...
	G_bprint(2, "%s %-15s \133%s\135 by %s on %s\n", icon,
			 dig3s("%s midair", rank), dig3s("%7.1f", midheight),
			 attacker->netname, targ->netname);

	if (attacker->ps.mid_total > 1) {
		attacker->ps.mid_totalheight += midheight;
		attacker->ps.mid_avgheight =
			attacker->ps.mid_totalheight / attacker->ps.mid_total;
	} else {
...
void T_Damage(gedict_t *targ, gedict_t *inflictor, gedict_t *attacker,
	...
	float non_hdp_damage; // save damage before handicap apply for kickback
						  // calculation
	...
	qbool midair = false, inwater = false, do_dmg = false, rl_dmg = false,
		  stomp_dmg = false;
	...
	if (isCA()) {
		...
		if ((targ->no_pain || (attacker->no_pain && attacker->in_play)) &&
			(match_in_progress == 2)) {
			if (attacker == targ) {
				tp4teamdmg = true; // don't take damage but still get
								   // stopped/bounced by weapon fire
			} else {
				if (targ->in_play &&
					(targ->invincible_sound < g_globalvars.time) &&
					strneq(targteam, attackerteam)) {
					sound(targ, CHAN_AUTO, "items/protect3.wav", 0.75,
						  ATTN_NORM);
	...
	// in teamplay 4 we do no armor or health damage to teammates (unless
	// telefrag), but do apply velocity changes
	if (tp_num() == 4 && streq(targteam, attackerteam) &&
		(isCA() || targ != attacker) && !TELEDEATH(targ)) {
		tp4teamdmg = true;
	}
	...
	if (midair) {
		inwater =
			(((int)targ->s.v.flags & FL_INWATER) && targ->s.v.waterlevel > 1);
		...
		if (!rl_dmg) {
			...
			do_dmg = targ->ct != ctPlayer // always do damage to non player,
										  // secret doors etc...
	...
	non_hdp_damage =
		damage; // save damage before handicap apply for kickback calculation

	// #handicap#
	if (attacker != targ) // attack no self
	{
		if ((attacker->ct == ctPlayer) &&
			(targ->ct == ctPlayer)) // player vs player
		{
			if ((hdp = GetHandicap(attacker)) !=
				100) // skip checks if hdp == 100
			{
				if ((dtAXE == targ->deathtype) || (dtSG == targ->deathtype) ||
					(dtSSG == targ->deathtype) || (dtNG == targ->deathtype) ||
					(dtSNG == targ->deathtype) || (dtGL == targ->deathtype) ||
					(dtRL == targ->deathtype) ||
	...
	virtual_take = max(0, take); // virtual_take used for calculating dmg_dealt
								 // only in case of k_dmgfrags
	...
	if (dtSUICIDE != targ->deathtype) {
				   ...
				   !TELEDEATH(targ)) // do telefrag damage in tp
		{
			...
			// teamplay == 3 don't damage mates, do damage to self (armor
			// affected anyway)
	...
	if (!match_in_progress && ((int)unbound_dmg_dealt > 0) &&
		(attacker->ct == ctPlayer) && (targ->ct == ctPlayer)) {
		...
		if (di && ((di == 2) || (attacker != targ))) {
			...
			stuffcmd_flags(attacker, STUFFCMD_IGNOREINDEMO,
						   "//ktx di %d %d %d %d %d %d %d\n",
	...
	if (save) {
		if ((streq(inflictor->classname, "worldspawn") ||
			 strnull(attacker->classname)) ||
			(targ->deathtype == dtWATER_DMG) ||
			(targ->deathtype == dtEXPLO_BOX) || (targ->deathtype == dtFALL) ||
	...
	if ((inflictor != world) &&
		...
		dir[2] = ((dtLG_DIS_SELF == targ->deathtype ||
				   dtLG_DIS == targ->deathtype) &&
				  dir[2] < 0)
					 ? -dir[2]
					 : dir[2];
		...
		for (i = 0; i < 3; i++) {
			targ->s.v.velocity[i] +=
				dir[i] * non_hdp_damage * c1 * nailkick *
				(midair && playerheight >= 45 ? (1 + (playerheight - 45) / 64)
											  : 1);
		}

		if (midair && (playerheight < 45)) {
			targ->s.v.velocity[2] +=
				dir[2] * non_hdp_damage * c2 * nailkick; // only for z component
		}
	...
	if ((match_in_progress == 2) &&
		((int)cvar("k_dmgfrags") || lgc_enabled())) {
	...
	if ((match_in_progress == 2) ||
		...
		|| (take >= 99999)) // do such huge damage even in prewar, prewar
							// because indirectly here match_in_progress != 2
	{
		...
		if (take) {
			...
			log_printf(
				...
				g_globalvars.time - match_start_time, cleantext(attackername),
				cleantext(victimname), death_type(targ->deathtype),
				(int)(attacker->super_damage_finished > g_globalvars.time ? 1
																		  : 0),
		...
		if (!targ->s.v.health || (dtSUICIDE == targ->deathtype)) {
			targ->s.v.health =
				-1; // qqshka, no zero health, heh, imo less bugs after this
		}
	}
	...
	// mid air bonuses
	if (midair && (match_in_progress == 2) && (attacker != targ) && take &&
		rl_dmg) {
	...
	if (ISDEAD(targ)) {
		// TODO: add the freeze here
		if (isCA() && cvar("k_clan_arena") == 3) {
			Frozen(targ, attacker, inflictor);
		} else {
...
void T_RadiusDamageApply(gedict_t *inflictor, gedict_t *attacker,
						 gedict_t *head, float damage, deathType_t dtype) {
	float points;
	vec3_t org;
	...
	if (head->s.v.takedamage) {
		...
		else if ((cvar("k_dis") == 2) && (dtLG_DIS == dtype) &&
				 !head->s.v.waterlevel) {
			points = 0;
		}
		...
		if (points > 0) {
			if (CanDamage(head, inflictor)) {
				...
				if (cvar("k_instagib") ||
					isRACE()) // in instagib splash applied to inflictor only,
							  // for coil jump
				{
					if (head == attacker) {
					...
					// shamblers only take half damage from rocket/grenade
					// explosions
